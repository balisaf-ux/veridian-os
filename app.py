...
with st.sidebar:
    ...
    if st.session_state.user_role == "ADMIN":
        ...
        industry_sim = st.selectbox("Vertical", ["Select...", "Industrial (S&R)", "Agri (Bonnyvale)", "Logistics (TTE)", "TTE Portal"])
...
if st.session_state.user_role == "ADMIN":
    ...
    elif industry_sim == "Industrial (S&R)": v_ind.render_liquid_module()
    elif industry_sim == "Agri (Bonnyvale)": v_ind.render_bonnyvale_module()
    elif industry_sim == "Logistics (TTE)": v_log.render_logistics_cloud()
    elif industry_sim == "TTE Portal": v_log.render_tte_portal()

elif st.session_state.user_role == "CLIENT":
    if menu == "Group Cockpit": v_admin.render_admin_home()
    elif menu == "Industrial": v_ind.render_liquid_module()
