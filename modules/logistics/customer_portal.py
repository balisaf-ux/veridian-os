...
# ==========================================================
# CUSTOMER PORTAL (SOVEREIGN BOOKING ENGINE)
# ==========================================================
...
def render_customer_portal():
    ...
    # ------------------------------------------------------
    if "tte_step" not in st.session_state:
        st.session_state.tte_step = 1

    if "demo_fill" not in st.session_state:
    ...
    st.progress(
        ...
        text=f"Step {st.session_state.tte_step} of 4"
    )

    # ======================================================
    # STEP 1 — CLIENT REGISTRATION & CREDIT CHECK
    # ======================================================
    if st.session_state.tte_step == 1:
        ...
        with st.form("client_reg"):
            ...
            c_type = c1.selectbox(
                "Client Sector",
                ["Government", "Corporate (Listed)", "Private/SME"]
            )
            ...
            c_terms = st.checkbox(
                "Apply for 30‑Day Payment Terms?",
                value=True if st.session_state.demo_fill else False
            )

            if st.form_submit_button("Submit Application"):

                # Sovereign credit logic
    ...
    # ======================================================
        ...
        with st.form("load_details"):
            ...
            origin = l1.selectbox(
                "Origin",
                ["Johannesburg", "Durban", "Cape Town", "Polokwane"]
            )
            dest = l2.selectbox(
                "Destination",
                ["Durban", "Cape Town", "East London", "Nelspruit"]
            )

            cargo = l1.text_input("Product Description", value=val_cargo)
            weight = l2.number_input("Load Weight (Tons)", 1, 34, 30)

            # Simple sovereign pricing model
