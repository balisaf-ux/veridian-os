...
def render_prospecting_dashboard():
    ...
    st.caption("Strategic Intelligence & Target Acquisition")

    # --- VISUAL COMMAND CENTER (NEW) ---
    st.markdown("### 🌍 Target Landscape")
    df_hunt = st.session_state.hunter_db
    
    # Handle missing columns if running on old session state
    ...
    # --- MAIN TABS ---
    tab_hunt, tab_engage, tab_pipe = st.tabs(["🦅 Hunter (Registry)", "🗣️ Engagement Log", "💰 DealStream (Pipeline)"])

    # --- TAB 1: HUNTER (INPUTS RESTORED) ---
    with tab_hunt:
        # A. INPUT FORM (NEW)
        with st.expander("🎯 ADD NEW PROSPECT (Input Intelligence)", expanded=False):
            with st.form("new_prospect_form"):
                ...
                p_summary = st.text_area("Business Summary & Context", placeholder="e.g., Large scale logistics operator facing efficiency challenges in KZN...")
        ...
        with c2:
            ...
            if st.button("🚀 Promote to DealStream"):
                ...
                new_deal = pd.DataFrame({
                    ...
                    'Stage': ["Lead"], 
                    'Value (ZAR)': [target_row['Turnover (ZAR)'] * 0.05], # Assume 5% Deal Value
                ...
                st.success(f"Successfully promoted {target_to_convert} to Active Pipeline!")
                time.sleep(1); st.rerun()

    # --- TAB 2: ENGAGEMENT LOG (PRESERVED) ---
    with tab_engage:
        ...
        with c_input:
            ...
            with st.form("log_int"):
                ...
                if st.form_submit_button("💾 Save to Log"):
                    new_log = pd.DataFrame({'Deal Name': [s_deal], 'Company': [s_deal], 'Contact Name': [c_name], 'Position': [c_pos], 'Email': [c_email], 'Phone': [c_phone], 'Date': [i_date], 'Type': [i_type], 'Notes': [i_notes]})
                    if 'Contact Name' not in st.session_state.activity_log.columns:
                        for col in ['Contact Name', 'Position', 'Email', 'Phone', 'Company']: st.session_state.activity_log[col] = None
        ...
        with c_history:
            ...
            if not st.session_state.activity_log.empty:
                ...
                for i, r in hist.iterrows():
                    ...
                    st.markdown(f"<div style='background:#f9f9f9; padding:15px; margin-bottom:10px; border-left:4px solid #0056b3; border-radius:5px;'><div style='font-weight:bold; font-size:1.1em; color:#2C3E50;'>{r['Deal Name']} <span style='font-weight:normal; font-size:0.8em; color:#7f8c8d;'> • {r['Type']}</span></div><div style='font-size:0.85em; color:#34495e; margin-bottom:5px;'>📅 {r['Date']} {contact_info}</div><div style='font-style:italic; color:#555;'>\"{r['Notes']}\"</div></div>", unsafe_allow_html=True)
            else:
                st.info("No engagement history found.")

    # --- TAB 3: DEALSTREAM (PRESERVED) ---
    with tab_pipe:
        st.subheader("💰 DealStream (Active Pipeline)")
        st.dataframe(st.session_state.deals_db[['Deal Name', 'Entity', 'Stage', 'Value (ZAR)', 'Probability', 'ES_Risk_Score']].style.background_gradient(subset=['Probability'], cmap="Blues"), use_container_width=True, hide_index=True, height=400)
